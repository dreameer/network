/*enderunix.org/docs/en/rawipspoof/
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
void send_syn(int sd)
{
	...
	ip.ip_p = IPPROTO_TCP;
	ip.ip_sum = 0x0;
	ip.ip_src.s_addr = inet_addr("10.1.3.31");
	ip.ip_dst.s_addr = inet_addr("10.180.203.234");
...
void raw_packet_receiver(u_char *udata,const struct pcap_pkthdr *pkthdr,const u_char *packet)
{
	...
	struct tcphdr *tcp;
	u_char *ptr;
	int ll_len = (int)udata;
	int s_seq;

	ip = (struct ip*)(packet +ll_len);
	tcp = (struct tcphdr*)(packet + ll_len + sizeof(struct ip));
...
{
	pcap_t *pd;
	char *filter = "dst host 10.1.3.31 and ip";
	char *dev = "vmx0";
	...
	if((pd = pcap_open_live(dev,1541,1,500,errbuf)) == NULL){
